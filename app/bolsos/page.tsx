...
import { getBolsos } from '../lib/products';
...
export default function BolsosPage() {
  const allProducts = getBolsos();
  const [showFilters, setShowFilters] = useState(false);

  // 1. Nuevo estado para el Ordenamiento
  const [sortOrder, setSortOrder] = useState('default');

  // 2. Estado para los Filtros (Color y Material)
  const [filters, setFilters] = useState({
    color: 'all',
    material: 'all',
  });

  // 3. Lógica Combinada: Primero Filtramos -> Luego Ordenamos
  const getProcessedProducts = () => {
    // A. FILTRADO
    const processed = allProducts.filter(product => {
      // Color filter
  ...
  return (
    <div className="container-custom py-12 min-h-screen">
      ...
      <motion.div
        ...
        variants={fadeInUp}
      >
        <h1 className="mb-4">Colección de Bolsos</h1>
        <p className="text-[var(--color-neutral)] max-w-3xl">
          Descubre nuestra colección completa de bolsos artesanales tejidos a mano.
          Cada pieza es única y está diseñada con dedicación y amor por el arte del crochet.
        </p>
      </motion.div>
      ...
      <div className="flex gap-8">
        ...
        >
          <div className="card p-6 sticky top-24">
            ...
            {/* Material */}
            <div className="mb-6">
              <label className="block mb-3 font-medium text-sm">Material</label>
              <Select
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="all">Todos</SelectItem>
                  <SelectItem value="Algodón Highlight">Algodón Highlight</SelectItem>
                  <SelectItem value="Cordones de algodón">Cordones de algodón</SelectItem>
                  <SelectItem value="Trapillo">Trapillo</SelectItem>
                  <SelectItem value="Cordones sintéticos">Cordones sintéticos</SelectItem>
                  <SelectItem value="Cola de rata">Cola de rata</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
        ...
        <div className="flex-1">
          ...
          >
            <p className="text-[var(--color-neutral)]">
              Mostrando {displayProducts.length} de {allProducts.length} productos
            </p>
          </motion.div>
